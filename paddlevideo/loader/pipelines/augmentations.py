...
class YowoAug(object):
    ...
    def random_distort_image(self, im, dhue, dsat, dexp):
        res = self.distort_image(im, dhue, dsat, dexp)
        return res
    
    def read_truths_args(self, lab_path, min_box_scale):
    ...
    def fill_truth_detection(self, labpath, flip, dx, dy, sx, sy):
        max_boxes = 50
    ...
    def __call__(self, results):
        ...
        if not self.valid_mode:
            ...
            clip = [self.random_distort_image(img, dhue, dsat, dexp) for img in sized]

            label = self.fill_truth_detection(labpath, flip, dx, dy, 1. / sx, 1. / sy)

        else:  # TODO Test tzw
            label = paddle.zeros([50*5])
            tmp = paddle.to_tensor(self.read_truths_args(labpath, 8.0/clip[0].width).astype('float32'))
